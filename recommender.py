...
pc = Pinecone(api_key=st.secrets["pinecone_api_key"])  #pcsk_xEske_CL1K1Kxm8Zu2ncNFPaVW9TKroYJHYJn7KYb6Vtug66GUF5q8mFPLN9JbpWkGhgP
...
def websearch(state:State):
  url=[]
  for i in state.meds:
    response = client.search(f"Show image of {i}, and also links to the product page",include_images=True) # Give only lorealparisusa websites
...
def age(state:State):
  ...
  img = load_img(state.image, target_size=(224, 224, 3))  # Changed to use 'state.image' instead of 'image_path'
  img = np.array(img)
