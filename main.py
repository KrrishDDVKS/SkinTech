...
if source == "Upload image":
    file = st.file_uploader("Upload a face image", type=["jpg", "jpeg", "png"], accept_multiple_files=False)
    if file is not None:
...
if uploaded_img is not None:
    st.success("Image acquired successfully. Proceeding to analysis...")
    gray = cv2.cvtColor(
        uploaded_img,
        cv2.COLOR_BGR2GRAY
    )
    ...
    if len(faces) == 0:
        st.error("No face detected in the image. Please upload / capture a clear face photo.")
        st.stop()
    elif len(faces) > 1:
        st.error(f"Multiple faces detected: {len(faces)}. Please upload / capture an image with only one face.")
        st.stop()
    else:
        st.success("Face detected successfully. Proceeding to quality check...")
        x, y, w, h = max(
        faces,
...
with left_col:
    ...
    st.image(face, caption="Your image", use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

    
    st.subheader("Image quality check")
    contrast_score, status = analyze_face_contrast_bgr(face)
                ...
                if isinstance(result, dict):
                    ...
                    if result["medimage"] == []:
                        ...
                        if isinstance(result, dict):
                            ...
                            if "bauman_type" in result:
                                st.info(f"**Bauman skin type:** {result['bauman_type']}")
                            if "des" in result:
                                st.markdown("### Analysis of Face to Recommend Products")
                                st.write(result["des"])
                            if "meds" in result and result["meds"]:
                                st.markdown("### Recommended products")
                                for url in result["meds"]:
                                    st.markdown(f"- {url}")
                            if "medimage" in result and result["medimage"]:
                                st.markdown("### Links to buy the products")
                                for url in result["medimage"]:
                                    st.markdown(f"- {url}")
                            if "toxic" in result and result["toxic"]:
                                st.markdown("### Ingredients of the products")
